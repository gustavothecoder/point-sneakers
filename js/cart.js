...
function GenerateCart() {
    ...
    if (cartDatabase.length === 0) {
        ...
        cartDatabase.forEach((order, index) => {
            ...
            $(`[product="${index}"]`).append(`<a href="#pages/sneaker-page.html" class="col-3 my-auto ml-3 ml-sm-5" cart-product-image="${index}" onclick="GenerateTheSneakerPage(${product.id})"></a>`, `<div class="sneaker-info col-3 my-auto" cart-product-info="${index}"></div>`, `<div class="sale-info col m-auto d-flex align-items-center" cart-sale-info="${index}"></div>`);
            ...
            if (product.sale) {
                $(`[cart-sale-info="${index}"]`).append(`<p class="mb-0" unit-price="${index}">$${product.newPrice}</p>`, `<input class="col-4 mx-3" type="number" name="quantity"  min="1" max="10" value="${order.quantity}" onchange="CalculatePriceCart(${index})" quantity-of-product="${index}">`, `<p class="mb-0" product-price="${index}">$${product.newPrice*order.quantity}</p>`);
            } else {
                $(`[cart-sale-info="${index}"]`).append(`<p class="mb-0" unit-price="${index}">$${product.price}</p>`, `<input class="col-4 mx-3" type="number" name="quantity"  min="1" max="10" value="${order.quantity}" onchange="CalculatePriceCart(${index})" quantity-of-product="${index}">`, `<p class="mb-0" product-price="${index}">$${product.price*order.quantity}</p>`);
            }
        });
    }
}
